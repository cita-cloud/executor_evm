...
use crate::core_executor::contracts::solc::allowance_management::Doornum;
use crate::core_executor::contracts::solc::permission_management::Resource;
...
/// Parse solidity return data `Address[], bytes4[]` to rust `Vec<Doornum>`
pub fn to_doornum_vec(output: &[u8]) -> Option<Vec<Doornum>> {
    // Decode the address[] and bytes4[]
    if output.is_empty() {
        Some(Vec::new())
    } else {
        ...
        .and_then(|(door_bytes, num_bytes)| {
            door_bytes
                .to_array()
                .map(|door_array| (door_array, num_bytes))
        })
        .and_then(|(door_array, num_bytes)| {
            num_bytes
                .to_array()
                .map(|num_array| (door_array, num_array))
        })
        .and_then(|(door_array, num_array)| {
            let mut v = Vec::new();
            for x in door_array {
                if let Some(x) = x.to_address() {
                    v.push(Address::from(x))
        ...
        .map(|(door_vec, num_vec)| {
            door_vec
                .into_iter()
                .zip(num_vec.into_iter())
                .map(|(cont, func)| Doornum::new(cont, func))
                .collect()
        })
    }
}
